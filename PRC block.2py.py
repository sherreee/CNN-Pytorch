...
def readData():
    ...
    data_path = "C:\Users\sherr\Desktop\NTU Y4S1\FYP\CNN Pytorch" # change according to your file path
    ex_data = np.loadtxt(data_path + "metadata.csv", # change according to your file name
                            delimiter=",")
    return ex_data

def rcImgProcessLs(train_images_array, # the train image array, with shape of (datasets_N, img_M*img_N)
                    ...
                    img_height = 256, img_width = 25): #image dimension to process, image width must be the multiplier of 3, i.e. 256 * 258, so padding is needed
